...
export class GraphComponent implements OnInit {
  ...
  async downloadPDF(): Promise<void> {
    ...
    pdf.setFontSize(10);
    pdf.text(
      'TOP BOOK BORROWERS BY DEPARTMENT AND GENDER',
      pdf.internal.pageSize.getWidth() / 2,
      100,
      { align: 'center' }
    );
  ...
  renderCharts(): void {
    ...
    const backgroundColors = Object.keys(this.departmentData).map(
      (department) => colorMapping[department]
    );
